...
export function CustomerSessions() {
  ...
  return (
    <details className="card-surface p-6">
      ...
      <div className="mt-5">
        {message && (
          <div className="mb-3 rounded-md border border-border p-3 text-sm text-muted-foreground">
            {message}
          </div>
        )}
        <div className="divide-y divide-border">
          {loading ? (
            ...
            <div className="py-4 text-sm text-muted-foreground">No active session.</div>
          ) : (
              ...
              >
                <div className="min-w-0">
                  <div className="text-sm font-medium">
                    {session.current ? "Current session" : "Customer session"}
                  </div>
                  <div className="truncate text-xs text-muted-foreground">
                    {session.user_agent || "Unknown device"}
                  </div>
                  <div className="mt-1 text-xs text-muted-foreground">
                    IP {session.ip_address || "unknown"} · Last seen{" "}
                    {new Date(session.last_seen_at).toLocaleString("en-US")}
                  </div>
                </div>
                <Button
                  type="button"
            ...
            onClick={() =>
              void (async () => {
                ...
                try {
                  ...
                  setMessage(value instanceof Error ? value.message : "Unable to revoke sessions.");
                } finally {
                  setLoading(false);
                }
              })()
